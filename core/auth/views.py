...
def register():
    # Don't allow a logged in user to try to register, redircet them to the tasks page.
...
def login():
    # Don't allow a logged in user to try to login, redirect them to the tasks page.
    nologin = False
    if current_user.is_authenticated:
        return redirect(url_for('task.tasks'))
